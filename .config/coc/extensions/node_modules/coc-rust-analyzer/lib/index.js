...
async function activate(context) {
    ...
    if (!bin) {
        ...
        if (ctx.config.prompt) {
            ret = await coc_nvim_1.window.showQuickpick(['Yes', 'Cancel'], msg);
        }
        if (ret === 0) {
            try {
    ...
    ctx.registerCommand('reload', (ctx) => {
        return async () => {
            coc_nvim_1.window.showMessage(`Reloading rust-analyzer...`);
            ...
            coc_nvim_1.window.showMessage(`Reloaded rust-analyzer`);
        };
    });
...
function analyzerStatus(ctx) {
    return async () => {
        ...
        coc_nvim_1.window.echoLines(ret.split('\n'));
    };
}
...
function joinLines(ctx) {
    return async () => {
        ...
        if (!range) {
            const state = await coc_nvim_1.workspace.getCurrentState();
            range = vscode_languageserver_protocol_1.Range.create(state.position, state.position);
        }
        const param = {
    ...
    return async () => {
        ...
        if (Array.isArray(response)) {
            ...
            if (vscode_languageserver_protocol_1.Location.is(location)) {
                uri = location.uri;
                pos = (_a = location.range) === null || _a === void 0 ? void 0 : _a.start;
            }
            else if (vscode_languageserver_protocol_1.LocationLink.is(location)) {
                uri = location.targetUri;
                pos = (_b = location.targetSelectionRange) === null || _b === void 0 ? void 0 : _b.start;
            }
        }
        else if (vscode_languageserver_protocol_1.Location.is(response)) {
            uri = response.uri;
            pos = response.range.start;
        }
        if (uri) {
            coc_nvim_1.workspace.jumpTo(uri, pos);
        }
    };
}
...
function ssr(ctx) {
    return async () => {
        const input = await coc_nvim_1.workspace.callAsync('input', ['Enter request like this: foo($a, $b) ==>> ($a).foo($b): ']);
        ...
        coc_nvim_1.window.withProgress({ title: 'Structured search replacing...', cancellable: false }, () => {
...
function serverVersion(ctx) {
    return async () => {
        ...
        coc_nvim_1.window.showMessage(version);
    };
}
...
function run(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(document))
            return;
        coc_nvim_1.window.showMessage(`Fetching runnable...`);
        const params = {
            textDocument: { uri: document.uri },
            position,
        };
        ...
        const idx = await coc_nvim_1.window.showQuickpick(items.map((o) => o.label));
        if (idx === -1) {
            return;
        }
...
function explainError(ctx) {
    return async () => {
        ...
        if (!ctx_1.isRustDocument(document))
            return;
        const diagnostic = (_b = (_a = ctx.client.diagnostics) === null || _a === void 0 ? void 0 : _a.get(document.uri)) === null || _b === void 0 ? void 0 : _b.find((diagnostic) => isInRange(diagnostic.range, position));
        if (diagnostic === null || diagnostic === void 0 ? void 0 : diagnostic.code) {
            ...
            const factory = new coc_nvim_1.FloatFactory(coc_nvim_1.workspace.nvim);
...
function toggleInlayHints(ctx) {
    return async () => {
        if (!ctx.config.inlayHints.chainingHints) {
            coc_nvim_1.window.showMessage(`Inlay hints for method chains is disabled. Toggle action does nothing;`, 'warning');
            return;
        }
...
async function applySnippetWorkspaceEdit(edit) {
    ...
    if (vscode_languageserver_protocol_1.TextDocumentEdit.is(change)) {
        ...
        if (selection) {
            ...
            await coc_nvim_1.window.moveTo(position);
        }
    }
}
...
function resolveCodeAction(ctx) {
    return async (params) => {
        ...
        if (!(item === null || item === void 0 ? void 0 : item.edit))
            return;
...
class Ctx {
    constructor(extCtx) {
        ...
        this.statusBar = coc_nvim_1.window.createStatusBarItem(10);
    ...
    registerCommand(name, factory, internal = false) {
        ...
        const d = coc_nvim_1.commands.registerCommand(fullName, cmd, null, internal);
    ...
    async startServer() {
        ...
        const client = client_1.createClient(bin, this.config.serverExtraEnv);
        ...
        client.onNotification(ra.status, async (params) => {
            ...
            if (status === 'ready') {
                ...
                const prompt = this.config.cargo.autoreload || (await coc_nvim_1.window.showPrompt(`rust-analyzer needs to reload project`));
                if (prompt) {
    ...
    resolveBin() {
        ...
        if (!executable_1.default.sync(bin)) {
            coc_nvim_1.window.showMessage(`${bin} is not executable`, 'error');
            return;
        }
        return bin;
    }
    async checkUpdate(auto = true) {
        ...
        if (old === latest.tag) {
            if (!auto) {
                coc_nvim_1.window.showMessage(`Your Rust Analyzer release is updated`);
            }
            return;
        }
        ...
        if (this.config.prompt) {
            ret = await coc_nvim_1.window.showQuickpick(['Yes, download the latest rust-analyzer', 'Check GitHub releases', 'Cancel'], msg);
        }
        if (ret === 0) {
            if (process.platform === 'win32') {
...
class ExperimentalFeatures {
    fillClientCapabilities(capabilities) {
        ...
        // @ts-ignore
        (_c = (_b = capabilities.workspace) === null || _b === void 0 ? void 0 : _b.workspaceEdit) === null || _c === void 0 ? void 0 : _c.normalizesLineEndings = true;
        // @ts-ignore
        (_e = (_d = capabilities.workspace) === null || _d === void 0 ? void 0 : _d.workspaceEdit) === null || _e === void 0 ? void 0 : _e.changeAnnotationSupport = {
            groupsOnLabel: true,
        };
        // @ts-ignore
        (_g = (_f = capabilities.textDocument) === null || _f === void 0 ? void 0 : _f.rename) === null || _g === void 0 ? void 0 : _g.prepareSupportDefaultBehavior = 1;
    }
    initialize() { }
    dispose() { }
}
function isSnippetEdit(action) {
    var _a, _b;
    for (const edit of (_b = (_a = action.edit) === null || _a === void 0 ? void 0 : _a.documentChanges) !== null && _b !== void 0 ? _b : []) {
...
function createClient(bin, extra) {
    var _a;
    let folder = '.';
    if ((_a = coc_nvim_1.workspace.workspaceFolder) === null || _a === void 0 ? void 0 : _a.uri.length) {
    ...
    const outputChannel = coc_nvim_1.window.createOutputChannel('Rust Analyzer Language Server Trace');
    const clientOptions = {
...
class Config {
    ...
    async onConfigChange(event) {
        ...
        const prompt = await coc_nvim_1.window.showPrompt(`${msg}. Reload now?`);
        if (prompt) {
    ...
    get serverPath() {
        var _a;
        return (_a = this.cfg.get('server.path')) !== null && _a !== void 0 ? _a : this.cfg.get('serverPath');
    }
    get serverExtraEnv() {
        var _a;
        return (_a = this.cfg.get('server.extraEnv')) !== null && _a !== void 0 ? _a : {};
    }
    get inlayHints() {
...
const zlib = __importStar(__webpack_require__(68));
...
async function getLatestRelease(updatesChannel) {
    ...
    // @ts-ignore
    const response = await node_fetch_1.default(releaseURL, { agent });
    if (!response.ok) {
...
async function downloadServer(context, release) {
    const statusItem = coc_nvim_1.window.createStatusBarItem(0, { progress: true });
...
class HintsUpdater {
    ...
    async activate() {
        ...
        const current = await coc_nvim_1.workspace.document;
        if (ctx_1.isRustDocument(current.textDocument)) {
...
exports.viewHir = new lc.RequestType('rust-analyzer/viewHir');
